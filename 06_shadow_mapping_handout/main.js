...
const cameraStartPos = vec3.fromValues(0, 1, -10);
...
function createSceneGraph(gl, resources) {
...

  {
    ...
    shadowNode.append(new TransformationSGNode(glm.transform({ translate: [0, -1.5, 0], rotateX: -90, scale: 3 }), [
      floor
    ]));
  }
...
function render(timeInMilliseconds) {
  ...
  const deltaTime = timeInMilliseconds - previousTime;
...
function initInteraction(canvas) {
  ...
  document.addEventListener('keypress', function (event) {
    ...
    if (event.code === 'KeyR') {
      camera.control.position = cameraStartPos;
